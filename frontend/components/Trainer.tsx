...
interface Props{ samples:Sample[]; onTrained:(m:tf.LayersModel)=>void; }
export default function Trainer({samples,onTrained}:Props){
  const[status,setStatus]=useState('Esperando...');
  async function train(){
    const labelled=samples.filter(s=>s.label);
    if(labelled.length<4){setStatus('Se necesitan ≥4 muestras');return;}
    ...
    const xT=tf.tensor2d(xs); const yT=tf.oneHot(tf.tensor1d(ys,'int32'),labels.length);
    ...
    (model as any).labels=labels; setStatus('Listo ✔'); onTrained(model);
  }
  return <div><button onClick={train}>Entrenar</button> <span>{status}</span></div>;
}
